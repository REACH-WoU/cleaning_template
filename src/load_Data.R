...
if(length(raw_data_filename) > 1) { stop("Found multiple files containing raw Kobo data! Please clean up the kobo_export folder.")
  ...
  kobo.raw.main <- readxl::read_xlsx(raw_data_filename, col_types = "text", sheet = 1) %>%
    rename(uuid = "_uuid")
  kobo.raw.loop1 <- readxl::read_xlsx(raw_data_filename, col_types = "text", sheet = 2) %>% 
    rename(uuid = "_submission__uuid",
           loop_index = "_index",
           parent_index = "_parent_index") %>% 
    mutate(loop_index = paste0("loop1_", loop_index))
  kobo.raw.loop2 <- readxl::read_xlsx(raw_data_filename, col_types = "text", sheet = 3) %>% 
    rename(uuid = "_submission__uuid",
           loop_index = "_index",
           parent_index = "_parent_index") %>% 
    mutate(loop_index = paste0("loop2_", loop_index))
  kobo.raw.loop3 <- readxl::read_xlsx(raw_data_filename, col_types = "text", sheet = 4) %>% 
    rename(uuid = "_submission__uuid",
           loop_index = "_index",
           parent_index = "_parent_index") %>% 
    mutate(loop_index = paste0("loop3_", loop_index))
