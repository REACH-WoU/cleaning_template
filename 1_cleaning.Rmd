...
pre_process_audit_files <- T # whether cases of respondent taking too long to answer 1 question should cleaned.
...
other_request_file_name <- "other_requests_final"
text_request_file_name <- "text_requests_final"

# if you clean your data during the round of their collection, each time adding new samples
partial_clean <- FALSE
other_request_file_pattern <- "other_requests_"
text_request_file_pattern <- "text_requests_"
...
empty_cols <- apply(raw.main,2,function(x){all(is.na(x))}) %>% 
  data.frame() %>% 
